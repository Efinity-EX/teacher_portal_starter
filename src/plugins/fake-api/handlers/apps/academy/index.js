    ...
    const filteredCourses = db.courses.filter((course) => {
      return (
        (course.courseTitle.toLowerCase().includes(queryLowered) ||
                ...
                (label !== 'All Courses'
                  ? course.tags.toLocaleLowerCase() === label?.toLowerCase()
                  : true)
      )
    })
    ...
    if (sortByLocal) {
      if (sortByLocal === 'courseName') {
        filteredCourses.sort((a, b) => {
          if (orderByLocal === 'asc') return a.courseTitle.localeCompare(b.courseTitle)
      ...
      if (sortByLocal === 'progress') {
        filteredCourses.sort((a, b) => {
          ...
          else return b.completedTasks / b.totalTasks - a.completedTasks / a.totalTasks
        })
      }
    }
