...
export const handlerAppsEcommerce = [
    ...
    if (sortByLocal) {
      ...
      if (sortByLocal === 'category') {
        filteredProducts = filteredProducts.sort((a, b) => {
          ...
          else if (orderByLocal === 'desc') return a.category < b.category ? 1 : -1
  ...
  http.get('/api/apps/ecommerce/orders', ({ request }) => {
    ...
    const filterOrders = db.orderData
      .filter((order) => {
        return (
    ...
    if (sortByLocal) {
      ...
      if (sortByLocal === 'customers') {
        filterOrders.sort((a, b) => {
          if (orderByLocal === 'desc') return b.customer.localeCompare(a.customer)
  ...
  http.delete('/api/apps/ecommerce/orders/:id', ({ params }) => {
    ...
    if (orderIndex >= 0) db.orderData.splice(orderIndex, 1)
  ...
  http.get('/api/apps/ecommerce/customers', ({ request }) => {
    ...
    const filteredCustomers = db.customerData
      .filter((customer) => {
        return (
    ...
    if (sortByLocal) {
      if (sortByLocal === 'users') {
        filteredReferrals.sort((a, b) => {
          if (orderByLocal === 'asc') return a.user.localeCompare(b.user)
