...
const { data: emailData, execute: fetchEmails } = await useApi(
  createUrl('/apps/email', {
    query: {
      q,
...
const isAllMarkRead = computed(() => {
  return selectedEmails.value.every(
    (emailId) => emails.value.find((email) => email.id === emailId)?.isRead
  )
})
...
const handleActionClick = async (action, emailIds = selectedEmails.value) => {
  ...
  if (action === 'trash') await updateEmails(emailIds, { isDeleted: true })
  else if (action === 'spam') await updateEmails(emailIds, { folder: 'spam' })
  else if (action === 'unread') await updateEmails(emailIds, { isRead: false })
  else if (action === 'read') await updateEmails(emailIds, { isRead: true })
  else if (action === 'star') await updateEmails(emailIds, { isStarred: true })
  else if (action === 'unstar') await updateEmails(emailIds, { isStarred: false })
  await fetchEmails()
  if (openedEmail.value) refreshOpenedEmail()
}
      ...
      >
        ...
        <div class="py-2 px-4 d-flex align-center d-flex gap-x-1">
          ...
          >
            ...
            <IconBtn
              v-show="
                'filter' in route.params ? route.params.filter !== 'trashed' : true
              "
              ...
              <VMenu activator="parent">
                <VList density="compact">
                  <VListItem
                    ...
                    @click="
                      async () => {
                        await updateEmailLabels(selectedEmails, label.title)
                        await fetchEmails()
                      }
                    "
                  >
